...
#======  input ===============================#
input_file_name <- 'case-Sv-pp-E-nu-1'
pars <- c('s11', 's12', 's13', 's22', 's23', 's33', 'p0', 'tv', 'ts', 'sigma', 'eigen_values', 'eigen_vectors')
...
E <- input$Young.GPa. #[GPa]
u <- input$Poisson 
K = E / (3.0*(1.0 - 2.0*u)) #bulk modulus [GPa]#
G = E / (2.0*(1.0 + u)) #shear modulus [GPa]#
...
data <- list(N = N, D = D, t = t, n = n, I = I, en = en, dt = dt, K = K, G = G, Sv = Sv, pp = pp)
