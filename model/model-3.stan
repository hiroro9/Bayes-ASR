data {
  ...
  real<lower=0> K;  //bulk modulus [GPa]
  real<lower=0> G; //shear modulus [GPa]
  real<lower=0> Sv; //Sv from density profile [MPa]
}
...
transformed parameters {
  vector[D] mu[N]; //mean of normal strain recovery [micro strain]
  vector[3] eigen_values; //magnitudes of principal stresses [MPa]
  matrix[3, 3] eigen_vectors; //orientations of principal stresses
  matrix[3, 3] s; //stress tensor
    s[1, 1] = s11;
    s[1, 2] = s12;
    s[1, 3] = s13;
    s[2, 1] = s12;
  ...
  for(i in 1:N){
    for(j in 1:D){
      mu[i, j] = (10^6)*((n[j]'*s*n[j] - (1.0/3.0)*trace(s))*(1 - exp(-t[i]/ts))/(2*G*10^3) 
      + ((1.0/3.0)*trace(s) - p0)*(1 - exp(-t[i]/tv))/(3*K*10^3));
    }
  }
...
model {
  ...
  //prior information for vertical stress
  s33 ~ normal(Sv, 5);
}
...
//  for(i in 1:N){
//    for(j in 1:D){
//      mu_new[i, j] = (10^6)*((n[j]'*s*n[j] - (1.0/3.0)*trace(s))*(1 - exp(-t[i]/ts))/(2*G*10^4) 
//      + ((1.0/3.0)*trace(s) - p0)*(1 - exp(-t[i]/tv))/(3*K*10^4));
//    }
//    en_new[i] = multi_normal_rng(mu_new[i], sigma*I);
//  }
//}
